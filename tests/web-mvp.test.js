...
test('browser preview adds a cabinet-style rope home with isolated rope states and global search', () => {
  ...
  assert.ok(js.includes('let activeHomePull = null;'));
  ...
  assert.ok(js.includes('function cancelHomePullDrag('));
  assert.ok(js.includes('const transitionPrepared = primeRopeTransitionView(ropeId);'));
  assert.ok(js.includes('completePrimedRopeTransition(pull.ropeId)'));
  ...
  assert.ok(js.includes("homePage.style.setProperty('--pull-drag-y'"));
  assert.ok(js.includes("homePage.style.setProperty('--pull-focus-local-x'"));
  assert.ok(js.includes("homePage.style.setProperty('--pull-focus-local-y'"));
  ...
  assert.ok(js.includes('}, 1740);'));
  assert.ok(js.includes('}, 360);'));
