...
test('mini add-rope layout follows the static index reference positions', () => {
  ...
  assert.ok(cssBlockLast('.page').includes('padding: 0;'));
  assert.ok(cssBlockLast('.rope-add-page').includes('display: flex;'));
  assert.ok(cssBlockLast('.add-rope-board').includes('width: 750rpx;'));
  assert.ok(cssBlockLast('.add-rope-board').includes('height: 100vh;'));
  assert.ok(cssBlockLast('.add-rope-board').includes('border: 0;'));
  assert.ok(cssBlockLast('.add-rope-board').includes('border-radius: 0;'));
  assert.ok(backButton.includes('top: 112rpx'));
  assert.ok(backButton.includes('left: 0'));
  assert.ok(backButton.includes('display: flex'));
  assert.ok(backButton.includes('flex-direction: column'));
  assert.ok(backButton.includes('align-items: center'));
  assert.ok(backButton.includes('justify-content: center'));
  assert.ok(backButton.includes('width: 74rpx'));
  assert.ok(backButton.includes('height: 128rpx'));
  ...
  assert.ok(!backButton.includes('width: 112rpx'));
  assert.ok(!backButton.includes('top: 150rpx'));
  assert.ok(!backButton.includes('height: 92rpx'));
  assert.ok(backTape.includes('top: -14rpx'));
  assert.ok(backTape.includes('left: 12rpx'));
  assert.ok(backTape.includes('width: 54rpx'));
  assert.ok(backTape.includes('height: 24rpx'));
  assert.ok(backTape.includes('transform: rotate(-7deg)'));
  assert.ok(cssBlockLast('.add-rope-back-char').includes('display: block'));
  assert.ok(cssBlockLast('.add-rope-back-char').includes('line-height: 1.22'));
  assert.ok(cssBlockLast('.add-rope-back-char').includes('font-size: 28rpx'));
  assert.ok(cssBlockLast('.add-rope-back-char + .add-rope-back-char').includes('margin-top: 2rpx'));
  ...
  assert.ok(cssBlockLast('.add-rope-rope').includes('height: 390rpx'));
  assert.ok(cssBlockLast('.add-rope-rope').includes('margin: 24rpx auto 0'));
  assert.ok(cssBlockLast('.add-rope-tail-image').includes('width: 96rpx'));
  assert.ok(cssBlockLast('.add-rope-tail-image').includes('height: 390rpx'));
  ...
  assert.ok(cssBlockLast('.create-rope-action').includes('margin: 14rpx auto 0'));
